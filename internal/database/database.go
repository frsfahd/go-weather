...
type Service interface {
	SetString(string, interface{}) error
...
func (s *service) SetString(key string, data interface{}) error {
	err := s.db.Set(context.Background(), key, data, time.Duration(cache_ttl)*time.Hour).Err()
	if err != nil {
		return err
	}
...
func (s *service) GetString(key string) (string, error) {
	data, err := s.db.Get(context.Background(), key).Result()
	if err != nil {
		return "", err
	}
